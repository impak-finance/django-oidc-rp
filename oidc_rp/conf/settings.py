...
# The 'USERINFO_EMAIL_KEY' allows specifying which key should be used to look up user email in the
# userinfo response, in case it's different than the standard.
USERINFO_EMAIL_KEY = getattr(settings, 'OIDC_RP_USERINFO_EMAIL_KEY', 'email')

# The 'UNAUTHENTICATED_SESSION_MANAGEMENT_KEY' settings defines a fixed string to use as a
# browser-state key for unauthenticated clients. It can be useful to define this value when it
...
FORCE_REDIRECT_URI_HTTPS = getattr(
    settings, 'OIDC_RP_FORCE_REDIRECT_URI_HTTPS', False
)
